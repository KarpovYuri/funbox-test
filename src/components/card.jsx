  ...
  return (
    <>
      ...
      >
        ...
        <div className={`oval ${isSelected ? 'oval_selected' : ''}`}>
          <p className='oval__weight'>{weight}</p>
          <p className='oval__unit'>кг</p>
        </div>
      </div>
      <p className='card__signature'>{isSelected ? signature : 'Чего сидишь? Порадуй, котэ, '}
        <a href='#'
          className={`card__link ${isSelected ? 'card__link_disabled' : ''}`}
          onClick={() => setIsSelected(true)}>
          купи
        </a>
        <span className={`card__point ${isSelected ? 'card__point_white' : ''}`}>.</span>
      </p>
    </>
  );
}
